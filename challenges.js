...
// Code Signal - arrayMaximalAdjacentDifference
...
function isIPv4Address(inputString) {
  ...
  for (let i = 0; i < splitStr.length; i++) {
      ...
      if (Number(splitStr[i]) >= 0 && Number(splitStr[i]) <= 255 && splitStr[i].length >= 1) {
          trueCounter++
      }
  }
  ...
  for (let j = 0; j < splitStr.length; j++) {
      ...
      if (splitStr[j].length > 1) {
                  splitIndex = splitStr[j].split('')
      }

      for (let k = 0; k < splitIndex.length; k++) {
          console.log(splitIndex[k])
          if (Number(splitIndex[k]) === 0 || Number(splitIndex[k - 1]) === 0) {
              zeroCounter++
          }
...
// Leet Code - Number of Steps to Reduce a Number to Zero
...
// Leet Code - Create Target Array in the Given Order
...
const createTargetArray = function(nums, index) {
  ...
  let target = []
  // iterate over the nums array using with the index being the value of the elements in the index array
  for (let i = 0; i < nums.length; i++) {
...
// Leet Code - Count of Matches in Tournament
...
const numberOfMatches = function(n) {
  let matches = 0, teams = n
  while (teams > 1) {
      matches += teams % 2 !== 0 ? (teams - 1) / 2 : (teams / 2)
      teams = teams % 2 !== 0 ? (teams - 1) / 2 + 1 : (teams / 2)
  }
  return matches
};